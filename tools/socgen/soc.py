            ...
            for x in range(0, self.noc.cols):
                ...
                if tile.ip_type.get() == tokens[2]:
                    for vf in range(self.noc.vf_points):
                        tile.energy_values.vf_points[vf].voltage = float(
                            tokens[3 + vf * 3])
                        tile.energy_values.vf_points[vf].frequency = float(
                            tokens[3 + vf * 3 + 1])
                        tile.energy_values.vf_points[vf].energy = float(
                            tokens[3 + vf * 3 + 2])
        return 0
        ...
        for y in range(self.noc.rows):
            for x in range(self.noc.cols):
                ...
                if self.IPs.ACCELERATORS.count(selection) == 0:
                    ...
                    for vf in range(self.noc.vf_points):
                        fp.write(str(tile.energy_values.vf_points[vf].voltage) +
                                 " " +
                                 str(tile.energy_values.vf_points[vf].frequency) +
                                 " " +
                                 str(tile.energy_values.vf_points[vf].energy) +
                                 " ")
                    fp.write("\n")
