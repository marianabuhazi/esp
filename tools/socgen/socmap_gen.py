...
def print_constants(fp, soc, esp_config):
    ...
    fp.write(
        "  constant CFG_IOLINK_BITS : integer := " +
        str(IOLINK_BITS) +
        ";\n")
...
def print_mapping(fp, soc, esp_config):
    ...
    fp.write("  type domain_type_array is array (0 to " +
             str(esp_config.ndomain - 1) + ") of integer;\n")
    ...
    for i in range(0, esp_config.ntiles):
        ...
        if t.type == "cpu" and t.has_pll != 0:
            fp.write("    " + str(i) + " => " +
                     str(esp_config.tiles[i].dvfs.idx) + ",\n")
    fp.write("    others => 0);\n\n")
    ...
    fp.write("  constant domains_num : integer := " +
             str(esp_config.ndomain) + ";\n\n")
    ...
    if esp_config.has_dvfs:
        for i in range(0, esp_config.ntiles):
            fp.write("    " + str(i) + " => " +
                     str(esp_config.tiles[i].has_pll) + ",\n")
    fp.write("    others => 0);\n\n")
    ...
    if esp_config.has_dvfs:
        for i in range(0, esp_config.ntiles):
            fp.write("    " + str(i) + " => " +
                     str(esp_config.tiles[i].clk_region) + ",\n")
    fp.write("    others => 0);\n\n")
    ...
    for i in range(0, esp_config.ntiles):
        fp.write("    " + str(i) + " => " +
                 str(pll_tile[esp_config.tiles[i].clk_region]) + ",\n")
    fp.write("    others => 0);\n\n")
    ...
    for i in range(0, esp_config.ntiles):
        fp.write("    " + str(i) + " => " +
                 str(esp_config.tiles[i].has_clkbuf) + ",\n")
    fp.write("    others => 0);\n\n")
    ...
    for i in range(0, esp_config.ntiles):
        ...
        # 9-12 - Processors' private cache controller (must change with
        # NCPU_MAX)
        if t.type == "cpu":
            if t.has_pll != 0:
